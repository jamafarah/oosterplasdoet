...
@section('content')
    ...
    <div class="banner row-3 orange-background" style="background-image: url(http://www.oosterplasdoet.nl/wp-content/uploads/2016/03/cropped-IMG_7544-1.jpg);">
        ...
        <h1 class="white">Beheer Sponsoren en Partners</h1>
    </div>
            ...
            <table class="overview">
                ...
                @foreach ($sponsors AS $sponsors)
                    <tr>
                        <td> {!! $sponsors->naam !!} </td>
                        <td> {!! $sponsors->type !!} </td>
                        <td><center> <a class="glyphicon glyphicon-pencil"  href="{{ route('changesponsor', $sponsors->id) }}"> Aanpassen</a></center> </td>
                        <td> <center><a class="glyphicon glyphicon-remove-sign"  href="{{ route('deletesponsor', $sponsors->id) }}"> Verwijderen</a></center> </td>
                    </tr>
                @endforeach
            </table>
        @else
